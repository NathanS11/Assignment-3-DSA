...
public class BinaryTree<T> implements BinaryTreeInterface<T>
{
    ...
    private void privateSetTree(T rootData,
        ...
        if ((leftTree != null) && !leftTree.isEmpty()) {
            BinaryNode<T> leftRoot = leftTree.root.copy(root);
            root.setLeftChild(leftRoot);
            // ADD CODE TO SET THE PARENT AND THREAD OF THE LEFT CHILD
            leftRoot.linkSubtreeThreadOut(root);
        }

        if ((rightTree != null) && !rightTree.isEmpty()) {
            BinaryNode<T> rightRoot = rightTree.root.copy(root);
            root.setRightChild(rightRoot);
            root.setThread(rightRoot.getLeftmostInSubtree());
